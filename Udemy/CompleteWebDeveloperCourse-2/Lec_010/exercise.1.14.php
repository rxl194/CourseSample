...
<nav class="navbar navbar-default navbar-static-top navbar-padded text-uppercase app-navbar">
  <div class="container">
    ...
    <div class="navbar-collapse collapse" id="navbar-collapse">
        ...
        <a target=_blank href="/createexercise.php?clone=1&code=1.14" class="btn btn-success navbar-right" style="margin-left:10px; padding:2px 10px 3px 10px;">Clone this exercise</a>    </div><!--/.nav-collapse -->
  </div>
</nav>

    <div class="container"><h1>Links</h1>
        
<div id="message"></div>
        
        <p>INSTRUCTIONS: Update the page to add another link to your favourite news site and also link the anchor tag in the last a href tag back to the BBC link.</p>
...
&lt;/head&gt;
    &lt;body&gt;
    	&lt;p&gt;&lt;a href=&quot;http://bbc.co.uk&quot;&gt;BBC Website&lt;/a&gt;&lt;/p&gt;
...
ifrm.document.close();
             ...
             if ($("iframe").contents().find("a").length == 4) {} else error = "It looks like you\'re missing an a tag, try again!";
    if ($("iframe").contents().find("p").length > 0 && $("iframe").contents().find("p").is("#bbc")) {} else error = "It looks like you\'re missing the ID for the anchor tag, try again!";
    if ($("iframe").contents().find("p").length == 4) {} else error = "It looks like you\'re missing a p tag, try again!";
                 if (error == "") $("#message").html('<div class="alert alert-success">Right on! You got it right, congratulations!  <a target=_blank href="https://twitter.com/intent/tweet?url=http://coding-exercises.com/exercise.php?code=1.14&text=I completed this coding challenge - can you?"><img src="tweet.png" alt="Tweet"></a> <a target=_blank href="https://facebook.com/sharer.php?u=http://coding-exercises.com/exercise.php?code=1.14"><img src="facebook.png" alt="Facebook"></a></div>'); else $("#message").html('<div class="alert alert-danger"><strong>There was an error in your code</strong>:<br>'+error+'</div>');
