...
<nav class="navbar navbar-default navbar-static-top navbar-padded text-uppercase app-navbar">
  <div class="container">
    ...
    <div class="navbar-collapse collapse" id="navbar-collapse">
        ...
        <a target=_blank href="/createexercise.php?clone=1&code=1.13" class="btn btn-success navbar-right" style="margin-left:10px; padding:2px 10px 3px 10px;">Clone this exercise</a>    </div><!--/.nav-collapse -->
  </div>
</nav>

    <div class="container"><h1>Tables</h1>
        
<div id="message"></div>
        
        <p>INSTRUCTIONS: Create a table with 'First Name' and 'Surname' as two separate table head labels and your actual first name and surname in separate table data cells in the table body</p>
...
ifrm.document.close();
             ...
             if ($("iframe").contents().find("table").length) {} else error = "You appear to have removed the table tag, you\'ll need to add this back in.";
    if ($("iframe").contents().find("td").length == 2) {} else error = "You\'re missing a td tag, have you added them both?";
    if ($("iframe").contents().find("th").length == 2) {} else error = "You\'re missing a th tag, have you added them both?";
    if ($("iframe").contents().find("tr").length == 2) {} else error = "You\'re missing a tr tag, have you added them both?";
    if ($("iframe").contents().find("tbody").length) {} else error = "You\'re missing a tbody tag, have you added this?";
    if ($("iframe").contents().find("thead").length) {} else error = "You\'re missing a thead tag, have you added this?";
                 if (error == "") $("#message").html('<div class="alert alert-success">Right on! You got it right, congratulations!  <a target=_blank href="https://twitter.com/intent/tweet?url=http://www.coding-exercises.com/exercise.php?code=1.13&text=I completed this coding challenge - can you?"><img src="tweet.png" alt="Tweet"></a> <a target=_blank href="https://facebook.com/sharer.php?u=http://www.coding-exercises.com/exercise.php?code=1.13"><img src="facebook.png" alt="Facebook"></a></div>'); else $("#message").html('<div class="alert alert-danger"><strong>There was an error in your code</strong>:<br>'+error+'</div>');
